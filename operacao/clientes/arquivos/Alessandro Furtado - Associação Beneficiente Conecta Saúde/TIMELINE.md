...
## 02/08/2026 — Relatório revisado: logo real, seção de Localização, item 10 do plano concluído

- Corrigido no `ConectandoSaude_RelatorioMensal_02-08-26.html`: logo real da Real Vision (antes era texto) na capa e assinatura, link `realvisionmaps.com` clicável, assinatura com nome (Felipe Garcia) + email — mesmo padrão já usado no relatório mais recente do Solarium/Gabriel.
- Adicionada seção 06 "Localização" com cidade/estado dos visitantes (GA4): 73% das 33 sessões de julho vieram do Brasil, concentradas em São Paulo (estado) — São Bernardo do Campo (11), São Paulo capital (6), Guarulhos (3). Os 9 acessos de fora (França, Alemanha, Índia, Holanda, EUA) incluem endereços de datacenter (Falkenstein, Boardman/Oregon), sinalizados no relatório como provável tráfego de robô, não visitante real.
- Seção "Próximos passos" revisada com o Felipe: mantido item de divulgação do link; removido pedido de Google Meu Negócio (status não confirmado); reescrito o pedido de conteúdo deixando claro que Conectando Saúde envia o material e a Real Vision publica; adicionados 3 itens da auditoria CRO (depoimentos, mais fotos/vídeo, mais patrocinadores) — os demais achados da auditoria (CNPJ placeholder, seção "Doe Agora" dedicada, fallback do formulário de voluntariado) ficam registrados só na `AUDITORIA-CRO-CONECTANDO-SAUDE.md`, não entraram no relatório por decisão do Felipe.
- Item 10 do `PLANO-EXECUCAO-PROXIMA-SESSAO.md` concluído: pedido de material entrou dentro do relatório mensal (seção "Ajustes feitos no site" + callout "Precisamos de você"), não como e-mail solto.
- **Padrão consolidado:** Felipe aprovou o resultado — `ConectandoSaude_RelatorioMensal_02-08-26.html` vira o modelo de referência pros próximos relatórios mensais deste cliente. `relatorio-mensal-modelo.md` e `RELATORIOS.md` atualizados.
- Pendente: criar/atualizar o rascunho no Gmail com esse relatório revisado antes de enviar ao Alessandro (o rascunho `r-4247648090546932498` foi criado antes dessas correções).
