# RELATÓRIO – LANDING PAGE SOLARIUM AARAU

**Cliente:** Gabriel (Solarium Aarau)  
**Projeto:** Landing Page para campanhas Meta Ads  
**Responsável:** Real Vision (Felipe Garcia)  
**Data:** 2026-06-24

---

## 1. Contexto Inicial

Gabriel enviou uma mensagem solicitando uma landing page para campanhas de Meta Ads.

**Análise realizada:**
- Site atual do Solarium Aarau foi completamente auditado
- Conclusão: funciona como website institucional e informativo

**O que o site atual explica:**
- Solarium e Collarium
- Equipamentos Ergoline
- Vitamina D
- Funcionamento do sistema de auto-atendimento

**Constatação importante:**
O site atual NÃO foi construído especificamente para conversão de tráfego frio proveniente de anúncios Meta.

**Estratégia definida:**
A ideia não é substituir o site atual, mas criar uma **landing page focada em conversão** que trabalhe em paralelo com o site institucional.

---

## 2. Objetivo Comercial Identificado

**Pergunta feita ao Gabriel:**
Qual é o objetivo principal da campanha?

**Resposta:**
"Mais clientes novos."

**Conclusão:**
O objetivo da landing page é **gerar novas visitas ao Solarium Aarau através de campanhas Meta Ads**.

---

## 3. Descoberta Importante Sobre o Modelo de Negócio

### Cenário Inicial Considerado
- WhatsApp
- Reserva online
- Formulários

### Realidade do Negócio (Esclarecimento)

O Solarium Aarau funciona da seguinte forma:
1. Cliente chega **sem reserva**
2. Compra créditos na máquina
3. Escolhe a máquina desejada
4. Utiliza o equipamento
5. **Não existe sistema de agendamento**

### Conclusão
A landing page **não precisa necessariamente focar em reservas**.

A estratégia de conversão será definida conforme a proposta final.

---

## 4. Estrutura Inicial de Landing Page Discutida

### Componentes sugeridos:

**Hero**
- Foto forte
- Headline impactante
- CTA principal claro

**Benefícios**
- Bronzeado perfeito
- Bem-estar e relaxamento
- Vitamina D
- Diferencial da Real Vision (Tour Virtual 360°)

**Prova Social**
- Google Reviews
- Depoimentos de clientes

**FAQ**
- Dúvidas comuns

**CTA Final**
- Botão de fechamento

### Observação Importante
Esta estrutura é apenas uma **referência inicial**.

O conteúdo definitivo será definido após alinhamento com o cliente.

---

## 5. Análise de Escopo

Identificado que antes de dar orçamento era necessário entender exatamente o escopo do projeto.

### 5.1 Conteúdo
**Decisão:** Utilizar material já existente do site atual.  
**Resultado:** Não será necessário produzir conteúdo totalmente novo.

### 5.2 Branding
**Decisão:** Utilizar identidade visual já existente.  
**Resultado:** Não haverá redesign da marca.

### 5.3 Fotos e Imagens
**Decisão:** Utilizar material já existente.  
**Resultado:** 9 fotos de estúdio + tour 360° já disponíveis.

### 5.4 Hospedagem
**Situação atual:** Site hospedado pela Real Vision.  
**Novo projeto:** Landing page será hospedada pela Real Vision Cloud.

**Ponto pendente — Confirmar com Gabriel:**
Em qual domínio a landing page será publicada?

**Possibilidades:**
- Mesmo domínio (www.aarau-solarium.ch/lp)
- Subdomínio (lp.aarau-solarium.ch)
- Endereço separado (novo domínio)

### 5.5 Tracking e Pixels
**Pendente confirmar com Gabriel:**
- Meta Pixel
- Google Analytics
- Conversões rastreadas

### 5.6 Gestão de Anúncios
**Situação:** Gabriel parece fazer seus próprios anúncios.  
**Necessário confirmar:** Se quer que Real Vision cuide também da gestão de campanhas.

### 5.7 Manutenção Pós-Entrega
**Necessário confirmar com Gabriel:**
- **Opção A:** Projeto único (entrega e fim)
- **Opção B:** Plano mensal (suporte + atualizações + manutenção)

---

## 6. Faixa de Preço Discutida

### Análise de Mercado
- **Landing page simples:** CHF 700 – CHF 1.200
- **Landing page profissional:** CHF 1.500 – CHF 2.500
- **Sistema completo (com ad management):** CHF 2.000 – CHF 4.000+

### Avaliação para Solarium Aarau
**Faixa provável:** CHF 1.000 – CHF 1.500

**Variáveis que podem alterar o preço:**
- Implementação de tracking completo
- Gestão de campanhas Meta
- Contrato de manutenção mensal
- Escopo final confirmado com Gabriel

---

## 7. Perguntas Definidas Para o Cliente

**Mensagem enviada em alemão (com Romana como co-responsável):**

> Hallo Gabriel,
> 
> Da wir aktuell das Konzept und die Offerte für die Landingpage ausarbeiten, möchte ich noch ein paar Punkte mit dir abstimmen, damit wir den genauen Umfang des Projekts festlegen können:
> 
> 1. **Unter welcher Domain soll die Landingpage laufen?** Möchtest du sie in die bestehende Website integrieren oder lieber als eigene Subdomain bzw. separate Adresse nutzen?
> 
> 2. **Möchtest du nur die Landingpage erstellen lassen** oder sollen wir zusätzlich auch das komplette Tracking für die Kampagnen einrichten (Meta Pixel, Google Analytics und Conversion-Tracking)?
> 
> 3. **Wirst du die Werbekampagnen selbst verwalten** oder sollen wir dies ebenfalls in die Offerte aufnehmen?
> 
> 4. **Bevorzugst du nach der Fertigstellung ein einmaliges Projekt** oder hättest du Interesse an einem monatlichen Paket für Support, Anpassungen und Wartung?
> 
> Sobald ich diese Informationen habe, kann ich dir eine vollständige Offerte vorbereiten.
> 
> **Zusätzlicher Hinweis:**
> Da Romana mich bei diesem Projekt unterstützt, werden wir die Abstimmung vorerst auf Deutsch machen. So kannst du alle Wünsche und Vorstellungen ganz unkompliziert auf Deutsch formulieren und wir können das Projekt von Anfang an sauber planen. 😊

---

## 8. Próximos Passos

### Fase 1: Aguardar Feedback
- ⏳ Respostas do Gabriel às 4 perguntas acima

### Fase 2: Após Receber Respostas
- 📋 Definir escopo final
- 💰 Definir valor exato
- 📝 Criar proposta comercial (PDF com Canva)
- 🎨 Criar estrutura detalhada da landing page
- 📅 Criar cronograma de desenvolvimento
- 🚀 Iniciar implementação

### Documentação para Retomada
Este documento representa o **estado atual do projeto** e pode servir como **ponto de retomada** para a próxima conversa.

---

## Notas Importantes

- **Envolvimento de Romana:** Co-responsável pela comunicação em alemão com Gabriel
- **Assets disponíveis:** 100% dos assets já existem no repositório
- **Reutilização de código:** Landing page usará componentes React existentes
- **Diferencial competitivo:** Tour 360° será destaque da landing page
- **Status:** Aguardando confirmação de escopo com Gabriel

---

**Versão:** 1.0  
**Data:** 2026-06-24  
**Responsáveis:** Felipe Garcia, Romana Loznjakovic

---

## 9. Resposta do Gabriel (17/06/2026)

**Mensagem recebida via WhatsApp:**

> "Oi, mano, desculpa por não ter respondido, estava com outras coisas na cabeça. Entendi sua mensagem, mesmo que ela tenha sido traduzida para o alemão por uma IA. É tudo muito complexo; gostaria de deixar por sua conta a forma como vamos organizar isso. Estou aberto a ideias e estratégias."

**Interpretação:** Gabriel é leigo e confia 100% na Real Vision para decidir escopo, domínio, tracking e manutenção. Não quer ser consultado para cada detalhe técnico.

---

## 10. Decisões Tomadas (28/06/2026) — por conta da Real Vision

Como Gabriel delegou as decisões, definimos o seguinte:

### 10.1 Domínio da Landing Page
**Decisão: rota `/willkommen` dentro do site atual** (aarau-solarium.ch/willkommen)
- Zero custo extra de domínio
- Mesmo repo (solariumaarau), mesmo Vercel — deploy automático
- SEO unificado com o site principal
- Header/footer suprimidos — página focada em conversão

### 10.2 Tracking
**Decisão: INCLUÍDO no escopo padrão** — Meta Pixel + Google Analytics 4 básico
- Sem tracking a campanha Meta Ads é cega: não é possível medir conversões
- Custo de implementação: baixo (código já documentado)
- GA4 e Meta Pixel são obrigatórios para qualquer campanha profissional

### 10.3 Gestão de Anúncios
**Decisão: NÃO incluída agora**
- Gabriel já gerencia seus próprios anúncios no Meta
- Gestão de campanhas é serviço separado (add-on mensal futuro, se ele pedir)
- Foco total na entrega da landing page primeiro

### 10.4 Manutenção Pós-Entrega
**Decisão: projeto único** (entrega + fim)
- Gabriel já paga anuidade de hospedagem (site + tour) — cobre infraestrutura
- Ajustes pontuais cobrados à parte quando necessário
- Sem contrato mensal fixo por enquanto

---

## 11. Histórico Financeiro do Gabriel (VisionFlow)

| Serviço | Valor | Status |
|---------|-------|--------|
| 2 Vídeos YouTube + Tour Virtual | 6.400 | ✅ Pago |
| Novo Site (aarau-solarium.ch) | 2.553 | ✅ Pago |
| Tour Correção | 638 | ✅ Pago |
| Anuidade Hospedagem (tour + site) | 1.021 | ✅ Pago |
| **Total investido** | **10.612** | ✅ |

*Valores em BRL (confirmado por Felipe em 28/06/2026). Convertendo a ~5,5 BRL/CHF = ~CHF 1.930 total investido.*

---

## 12. Escopo Final e Precificação da Landing Page

**O que está incluído:**
- Rota `/willkommen` no site existente (React/Vite/Tailwind)
- Hero com urgência (endereço, horário, "Jetzt geöffnet")
- 3 passos simplificados (SpotlightCard reutilizados)
- Prova social (3 Google Reviews reais)
- Tabela de Bônus de boas-vindas
- Preview do Tour 360°
- CTA final com Google Maps embed
- Meta Pixel + GA4 configurados
- Deploy em Vercel (URL provisória para aprovação do Gabriel)
- Copy em alemão (Romana revisa)

**O que NÃO está incluído:**
- Gestão de campanhas Meta Ads
- Contrato de manutenção mensal
- Novo domínio

**Preço: a definir com Romana** (referência Felipe: CHF 300)
- Felipe sugere CHF 300 como ponto de partida — Romana confirma antes do envio da proposta
- Escopo reduzido justifica valor menor: reutiliza 100% dos assets e componentes existentes
- Valores anteriores pagos (BRL): site completo R$2.553 / anuidade R$1.021

---

## 13. Próximos Passos (a partir de 28/06/2026)

| Passo | Responsável | Status |
|-------|-------------|--------|
| 1. Construir `/willkommen` no repo solariumaarau | Felipe | 🔄 Em andamento |
| 2. Deploy Vercel com URL provisória | Felipe | ⏳ Após desenvolvimento |
| 3. Implementar Meta Pixel + GA4 | Felipe | ⏳ Junto com dev |
| 4. Romana revisa copy alemão | Romana | ⏳ Paralelo ao dev |
| 5. Enviar link provisório + proposta CHF 1.200 ao Gabriel | Felipe/Romana | ⏳ Após deploy |
| 6. Aguardar aprovação do Gabriel | Gabriel | ⏳ |
| 7. Ajustes finais → deploy em produção | Felipe | ⏳ |

**Versão:** 2.0  
**Data:** 2026-06-28  
**Responsáveis:** Felipe Garcia, Romana Loznjakovic
