---
title: Eduardo Barqueiro — Paraty Onboard
tags:
  - cliente
  - entregue
status: entregue
data_inicio: 2026-06-16
servicos:
  - cartao-digital
---

# Eduardo Barqueiro — Paraty Onboard

Wiki: [[eduardo-barqueiro]]

## Contexto

Eduardo Barqueiro é dono/gestor da Paraty Onboard, empresa de passeios e transporte em Paraty (RJ). Chegou pela campanha de prospecção de pousadas/turismo de Paraty em junho/2026.

## Serviços contratados

- **Cartão Digital** (entregue) — landing page de link bio com informações da empresa

## Entregas realizadas

- **16/06/2026** — Cartão Digital entregue em `paraty.realvisionmaps.com` (subdomínio Hostinger via CNAME)
  - Stack: Vite + React + Tailwind + glassmorphism
  - Repo privado no GitHub → deploy Vercel → CNAME Hostinger

## Próximos passos

- [ ] Follow-up pós-entrega: satisfação com o cartão
- [ ] Oportunidade: Tour Virtual 360° para a frota/operação da Paraty Onboard

## Observações

- Primeiro cartão digital entregue pela Real Vision (caso de referência para o produto)
- Contato via WhatsApp (campanha de prospecção)
- Localização: Paraty, RJ
