...
* **Construindo ou consertando um portão de login/rota protegida?** Ver skill `rv-portao-auth` — permissão tem três estados (`checando`/`ok`/`negado`/`erro`), nunca um booleano, e "ainda não sei" jamais pode ser tratado como "não". É o defeito que gera loop entre login e painel com o console limpo.
