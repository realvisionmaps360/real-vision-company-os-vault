...
> **Nota de escopo Real Vision (12/07/2026, atualizada 18/08/2026):** dentro do contexto Real Vision, esta skill não é mais o ponto de entrada padrão pra "handoff"/"session handoff" — quem recebe esse gatilho primeiro é [[rv-fim-sessao]], que decide o que gravar permanentemente (FICHA-CLIENTE/TIMELINE de cliente, ou TIMELINE/playbook de projeto interno) e só invoca este bilhete efêmero como complemento, se sobrar trabalho técnico pela metade. O comportamento desta skill em si não mudou — continua chat-only, nunca grava arquivo. A única adição é que, quando há uma sessão de destino nomeada, o `rv-fim-sessao` também manda o mesmo bilhete direto pra ela via `send_message` (Passo 4 daquela skill) — isso é orquestração de fora, não uma mudança nas regras aqui. Fora do contexto Real Vision (outros projetos), esta skill funciona exatamente como sempre, sem essa camada.
